...
class ConfusionMatrix:
  ...
  def __init__(self, number_of_labels = 2, ignore_label=None):
  ...
  def count_predicted_batch(self, ground_truth_vec, predicted): # added
    if self.ignore_label is not None:
      for i in range(ground_truth_vec.shape[0]):
        ...
        elif i > self.ignore_label:
          self.confusion_matrix[:,predicted[i-1]] += ground_truth_vec[i-1,:]
    else:
      for i in range(ground_truth_vec.shape[0]):
        self.confusion_matrix[:,predicted[i]] += ground_truth_vec[i,:]
  ...
  def get_per_class_accuracy(self, i):  # added
    # re = []
    # for i in range(self.number_of_labels):
    re=self.confusion_matrix[i][i] /  max(1,np.sum(self.confusion_matrix[i,:]))
    return re
